...
if [[ "$RUN_ON_REMOTE" == "1" ]]; then
	if [[ -z "$DOMAIN" || -z "$EMAIL" ]]; then
		echo "Missing required args in --run-on-remote mode. Use --domain and --email."
		exit 1
	fi
else
...
if [[ "$RUN_ON_REMOTE" == "1" ]]; then
	echo "Starting local-on-remote recovery for ${DOMAIN}"
	echo "Deployment identity: app_slug=${APP_SLUG} pm2=${PM2_APP_NAME} project_dir=${PROJECT_DIR}"
	bash -s -- "$DOMAIN" "$EMAIL" "$REPO_URL" "$BRANCH" "$PROJECT_DIR" "$PM2_APP_NAME" "$BACKEND_PORT" "$ENV_SYNC_DIR" <<'REMOTE_SCRIPT'
set -euo pipefail
